                    ...
                    for dj in (-1,0,1):
                        if di !=0 or dj!=0:
                            ...
                            if within_grid(newi, newj, skel.shape[0], skel.shape[1]):
                                if skel[newi, newj]:
                                    graph.add_edge((i,j), (newi, newj), weight = math.sqrt(di*di+dj*dj))
    return graph
        ...
        for e in p_edges:
            ...
            if q in major_nodes:
                ...
                while True:
                    ...
                    if not all_on_straight_line(np.array(p),
                                                np.array(q),
                                                in_between,
                                                tol):
                        q = in_between.pop()
...
def all_on_straight_line(p: np.ndarray, q:np.ndarray, in_between: List[Tuple[float, float]], tol: float):
    d = p-q
